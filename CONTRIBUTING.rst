...
Everyone interacting in the fileviewer project's code base,
...
Report bugs at https://github.com/goerz/fileviewer/issues.
...
The best way to send feedback is to file an issue at https://github.com/goerz/fileviewer/issues.
...
3. Check https://travis-ci.org/goerz/fileviewer/pull_requests
...
Ready to contribute? Follow `Aaron Meurer's Git Workflow Notes`_ (with ``goerz/fileviewer`` instead of ``sympy/sympy``)

In short,

1. Clone the repository from ``git@github.com:goerz/fileviewer.git``
...
fileviewer uses a simple branching model where all
...
fileviewer includes a full test-suite using pytest_.

From a checkout of the ``fileviewer`` repository  you can use
...
:mod:`fileviewer` package:

.. doctest::

    >>> import fileviewer
    >>> fileviewer.__version__   # doctest: +SKIP
...
https://github.com/goerz/fileviewer/releases.
